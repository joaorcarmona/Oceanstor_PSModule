function Enable-DMFileSystemReplicationPairSecondaryProtection {
    ...
    if ($PSCmdlet.ShouldProcess($Id, 'Enable file-system replication secondary protection')) {
        $response = Invoke-DeviceManager -WebSession $session -Method 'PUT' -Resource 'REPLICATIONPAIR/SET_SECONDARY_WRITE_LOCK' -BodyData $body
