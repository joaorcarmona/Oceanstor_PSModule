...
.DESCRIPTION
    Resolves a snapshot name or Id to its ID and activates it through the OceanStor snapshot REST resource.
    ...
    Accepts multiple snapshots from the pipeline by property name. Each is activated independently: a
...
.PARAMETER SnapShotName
    Name of the snapshot to activate. Valid values are checked against Get-DMLunSnapshot and support tab completion. Mutually exclusive with SnapShotId.

.PARAMETER SnapShotId
    Id of the snapshot to activate. Valid values are checked against Get-DMLunSnapshot, no tab completion. Mutually exclusive with SnapShotName.
...
.EXAMPLE
    PS> Get-DMLunSnapshot -Id 5 | Enable-DMLunSnapshot

    Activates the piped snapshot.
...
function Enable-DMLunSnapshot {
    ...
    process {
        try {
            ...
            if ($PSCmdlet.ShouldProcess($snapshot.Name, 'Activate LUN snapshot')) {
