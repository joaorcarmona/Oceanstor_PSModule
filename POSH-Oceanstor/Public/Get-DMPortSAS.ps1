...
.DESCRIPTION
    Function to request Huawei Oceanstor Storage configured SAS Ports. With no
    arguments, returns every SAS port. When Name is supplied (positionally or named),
...
.PARAMETER Name
    Optional SAS port name to search for, positional. If omitted, every SAS port is returned. Supports PowerShell wildcards (*, ?, [...]); without one, the comparison is an exact match.
...
.EXAMPLE
    ...
    PS C:\> Get-DMPortSAS 'CTE0.A.IOM0.P0'

    OR

    PS C:\> Get-DMPortSAS 'CTE0.A.IOM0.*'
...
#>
    ...
    if ($Name) {
        ...
        $sasPorts = [System.Collections.ArrayList]@($sasPorts | Where-Object Name -Like $Name)
    }
