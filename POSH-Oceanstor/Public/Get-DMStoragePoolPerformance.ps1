function Get-DMStoragePoolPerformance {
    <#
    .SYNOPSIS
        Retrieves realtime performance samples for OceanStor storage pools.

    .DESCRIPTION
        Thin wrapper around Get-DMPerformance for object type StoragePool. Accepts one or more
        OceanStorStoragePool objects via pipeline (e.g. from Get-DMstoragePool), batches all
    ...
    .PARAMETER InputObject
        One or more OceanStorStoragePool objects, pipeline-able.
    ...
    .EXAMPLE
        PS> Get-DMstoragePool | Get-DMStoragePoolPerformance

    .NOTES
        Filename: Get-DMStoragePoolPerformance.ps1
    #>
    [CmdletBinding()]
    ...
    process {
        foreach ($obj in $InputObject) {
            if ($obj -isnot [OceanStorStoragePool]) {
                throw "Get-DMStoragePoolPerformance: InputObject must be an OceanStorStoragePool object."
            }
            $ids.Add($obj.Id)
        }
    }
        ...
        $params = @{ WebSession = $session; ObjectType = 'StoragePool'; ObjectId = @($ids) }
