function New-DMSnapshotConsistencyGroup {
    ...
    param(
        ...
        [ValidateScript({
                ...
                if ($matchingItems.Count -gt 1) { throw "ProtectionGroupName is ambiguous because more than one protection group is named '$_'." }
    ...
    if ($protectionGroup.'vStore ID') { $body.vstoreId = $protectionGroup.'vStore ID' }
