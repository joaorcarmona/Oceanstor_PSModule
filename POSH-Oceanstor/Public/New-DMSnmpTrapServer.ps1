function New-DMSnmpTrapServer {
    ...
    if ($PSCmdlet.ShouldProcess($Address, 'Add SNMP trap server')) {
        $response = Invoke-DeviceManager -WebSession $session -Method 'POST' -Resource 'snmp_trap_addr' -BodyData $body
