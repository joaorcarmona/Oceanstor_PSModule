<#
.SYNOPSIS
    Removes a direct OceanStor host-to-LUN mapping.
...
.DESCRIPTION
    ...
    Accepts multiple hosts from the pipeline by property name. Each is resolved and unmapped from
    the same LUN independently: a failure (e.g. an invalid/ambiguous name, or a REST error) is
...
.PARAMETER HostName
    Name of the host to unmap from the LUN.

.PARAMETER LunName
    Name of the LUN to unmap the host from.
...
.EXAMPLE
    PS> Remove-DMHostFromMappingView -HostName 'esx01' -LunName 'data-lun' -WhatIf

    Shows what would happen if esx01 were unmapped from data-lun.

.NOTES
    Filename: Remove-DMHostFromMappingView.ps1
#>
function Remove-DMHostFromMappingView {
    ...
    process {
        try {
            ...
            if ($PSCmdlet.ShouldProcess("$HostName <- $LunName", 'Remove host-LUN mapping')) {
