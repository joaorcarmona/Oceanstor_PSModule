function Remove-DMReplicationConsistencyGroup {
    ...
    if ($PSCmdlet.ShouldProcess($groupId, 'Remove remote replication consistency group')) {
        $response = Invoke-DeviceManager -WebSession $session -Method 'DELETE' -Resource "CONSISTENTGROUP/$groupId"
