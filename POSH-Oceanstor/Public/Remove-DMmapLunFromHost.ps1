...
.DESCRIPTION
    Deletes a direct host-to-LUN mapping via the OceanStor mapping interface. Both LunName and
    ...
    the same host independently: a REST error is reported as a non-terminating error and does not
...
.PARAMETER LunName
    Name of the LUN to unmap from the host. Validated against existing LUNs and supports tab completion.

.PARAMETER HostName
    Name of the host to unmap the LUN from. Validated against existing hosts and supports tab completion.
