function Start-DMHyperMetroConsistencyGroup {
    ...
    if ($PSCmdlet.ShouldProcess($groupId, 'Start HyperMetro consistency group')) {
        $response = Invoke-DeviceManager -WebSession $session -Method 'PUT' -Resource 'HyperMetro_ConsistentGroup/start' -BodyData @{ ID = $groupId }
